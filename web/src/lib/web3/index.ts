...
import {StratagemsWarsAccountData} from '$lib/account/account-data';
...
const logger = logs('stratagems-wars');
...
export const accountData = new StratagemsWarsAccountData();
...
// But note that Stratagems-Wars is designed to work without external RPC
				...
				if (!signature) {
					async function signMessage() {
						const msg = stringToHex(
							'Welcome to Stratagems-Wars, Please sign this message only on trusted frontend. This gives access to your local data that you are supposed to keep secret.',
						);
